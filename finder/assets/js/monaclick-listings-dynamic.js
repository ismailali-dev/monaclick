(() => {
  ...
  const moduleConfig = (() => {
    ...
    if (selectedModule === 'cars') {
      const activeFiltersRow = document.querySelector(
        '.d-flex.align-items-center.gap-3.border-bottom.pb-2.mb-4'
      );
      const resultsText = activeFiltersRow?.querySelector('.fs-sm.text-nowrap');
      return {
        ...
        paginationNav: document.querySelector('.col-lg-9 nav[aria-label="Listings pagination"]'),
        searchInput: null,
        stateSelect: document.querySelector('select[data-location-state]'),
        citySelect: document.querySelector('select[data-location-city]'),
        categoryList: null,
        sortSelect: document.querySelector('.col-lg-9 .position-relative select'),
        clearAllLink: activeFiltersRow?.querySelector('a.nav-link.text-decoration-underline'),
        activePillsWrap: activeFiltersRow?.querySelector('.w-100.pb-3.overflow-x-auto .d-flex.gap-2'),
        gridMode: 'cols',
      };
    }
  ...
  const getStateFromUrl = () => {
    ...
    return {
      ...
      region: (params.get('state') || '').trim().toUpperCase(),
  ...
  const applyStateToUrl = () => {
    ...
    if (state.region) params.set('state', state.region);
  ...
  const loadListings = () => {
    ...
    if (state.region) apiQuery.set('state', state.region);
  ...
  if (stateSelect) {
    stateSelect.addEventListener('change', () => {
      state.region = String(stateSelect.value || '').trim().toUpperCase();
      state.city = '';
      state.page = 1;
  ...
  if (clearAllLink) {
    clearAllLink.addEventListener('click', (event) => {
      event.preventDefault();
      state = { q: '', region: '', city: '', category: '', sort: '', ratings: [], budgets: [], features: [], availability: false, page: 1 };
      syncControlsFromState();
      applyStateToUrl();
      loadListings();
    });
