(() => {
  window.__MC_ENTRY_DYNAMIC_VERSION__ = '2026-05-20-claim-r1';
  ...
  const renderEntry = (item, related = []) => {
    ...
    const claimUrl = item?.claim?.url || (item?.slug ? `/claim/${encodeURIComponent(item.module)}?slug=${encodeURIComponent(item.slug)}` : '');
    const claimButton = claimUrl
      ? `
          <li class="d-flex align-items-center gap-2 ms-2">
            <a class="btn btn-sm btn-primary" href="${escapeHtml(claimUrl)}">
              Claim Business
            </a>
          </li>
        `
