      ...
      <div class="container pb-md-2">
        ...
        <div class="accordion row pt-4 pt-sm-5 mt-3 mt-sm-0" id="footerLinks">
          <div class="col-md-4 col-lg-5 mb-4 mb-sm-5 mb-md-0">
            <a class="d-inline-flex align-items-center text-dark-emphasis text-decoration-none mb-3" href="/">
...
(() => {
  ...
  const setupUsLocationFields = () => {
    const stateSelect = document.getElementById('state')
      || document.querySelector('select[data-location-state]');
    if (!(stateSelect instanceof HTMLSelectElement)) return;

    const citySelect = document.getElementById('city')
      || document.querySelector('select[data-location-city]');
    if (!(citySelect instanceof HTMLSelectElement)) return;

    // These options are loaded asynchronously. Keep them as native Bootstrap
    // selects so Choices.js does not snapshot only the placeholder option.
    stateSelect.removeAttribute('data-select');
    citySelect.removeAttribute('data-select');
