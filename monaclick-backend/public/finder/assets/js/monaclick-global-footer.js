(() => {
  ...
  const bindAddListingAuthPrompt = () => {
    ...
    document.querySelectorAll('a[href="/add-listing"]').forEach((trigger) => {
      if (!(trigger instanceof HTMLAnchorElement)) return;
      trigger.setAttribute('data-mc-no-loader', '1');
    });
  ...
  const setupJoinProNetworkGate = () => {
    const triggers = Array.from(document.querySelectorAll('a, button'))
      .filter((el) => {
        ...
        return text === 'join pro network' || text === 'create a listing';
      });
    if (!triggers.length) return;

    triggers.forEach((trigger) => {
      if (!(trigger instanceof HTMLElement)) return;
      if (trigger.dataset.mcListingCtaNormalized !== '1') {
        trigger.dataset.mcListingCtaNormalized = '1';
        trigger.setAttribute('data-mc-no-loader', '1');
        if (trigger instanceof HTMLAnchorElement) {
          trigger.setAttribute('href', '/add-listing');
        }
        const icon = trigger.querySelector('i');
        trigger.textContent = '';
        if (icon) {
          trigger.appendChild(icon);
          icon.className = 'fi-plus fs-lg animate-target ms-n2 me-1 me-sm-2';
        }
        trigger.append('Create');
        const extra = document.createElement('span');
        extra.className = 'd-none d-xl-inline ms-1';
        extra.textContent = 'a listing';
